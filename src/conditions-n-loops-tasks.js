...
function canQueenCaptureKing(queen, king) {
  ...
  for (let i = 0; i < 9; i += 1) {
    queenXX += 1;
    queenYY -= 1;
    if (kingX === queenXX && kingY === queenYY) {
      return true;
    }
  }
  queenXX = queenX;
...
function convertToRomanNumerals(num) {
  ...
  let numVarieble = num;
  if (num <= 10) {
    resultAfterTen = `${checkBeforeTen(numVarieble)}`;
  }
  if (num <= 20 && num > 10) {
    numVarieble -= 10;
    resultAfterTen = `X${checkBeforeTen(numVarieble)}`;
  }
  if (num <= 30 && num > 20) {
    numVarieble -= 20;
    resultAfterTen = `XX${checkBeforeTen(numVarieble)}`;
  }
  if (num <= 39 && num > 30) {
    numVarieble -= 30;
    resultAfterTen = `XXX${checkBeforeTen(numVarieble)}`;
  }
  return resultAfterTen;
}
